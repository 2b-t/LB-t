    ...
    TYPED_TEST(DdQqTest, logicalMask) {
      ...
      for (std::int32_t i = 0; i <= 1; ++i) {
        for (std::int32_t j = 0; j < TypeParam::HSPEED; ++j) {
          ...
          if (index != hspeed) {
            is_success &= lbt::cem::isNearlyEqualEpsAbs(TypeParam::MASK.at(index), static_cast<typename TypeParam::type>(1));
          } else {
            is_success &= lbt::cem::isNearlyEqualEpsAbs(TypeParam::MASK.at(index), static_cast<typename TypeParam::type>(0));
          }
        }
      }
